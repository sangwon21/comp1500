    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            using (Stream write = File.OpenWrite($"{DECODE_TEST1}.output.txt"))
            {
                ...
                // Check test.output.output.txt. The content should be the same as test.txt
                Debug.Assert(bEncoded);
            }
            ...
            {
                bool bEncoded = Encoder.TryDecode(read, write);
                // Check test2.output.output.txt. The content should be the same as test2.txt
                Debug.Assert(bEncoded);
            }
            ...
            {
                bool bEncoded = Encoder.TryDecode(read, write);
                // Check empty.txt. The content should be empty.
                Debug.Assert(!bEncoded);
            }
        }
